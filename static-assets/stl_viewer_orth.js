...
function StlViewer(parent_element_obj, options)
{
	...
	// this.set_zoom = function (zoom, force_zoom)
	// {
	// 	if (_this.zoom_done && !force_zoom) return;
		
	// 	_this.zoom_done = true;
		
	// 	// Simple positioning for orthographic camera
	// 	_this.camera.position.set(0, 0, 50);
		
	// 	// Make sure camera looks at center
	// 	_this.camera.lookAt(0, 0, 0);
		
	// 	if (_this.controls && _this.controls.target) {
	...
	this.set_zoom = function (zoom, force_zoom)
	{
		...
		if ((_this.zoom_done) && (!force_zoom))
			return;
				
		_this.zoom_done = true;
		
		if (_this.camera instanceof THREE.OrthographicCamera) {
			var frustumHeight = _this.zoom;
			if (_this.zoom < 0) {
				frustumHeight = _this.calc_z_for_auto_zoom();
			}
			
			// Store current frustum height for resize calculations
			_this.currentFrustumHeight = frustumHeight;
			
			// Update camera frustum
			var aspect = _this.camera.right / _this.camera.top || 1;
			_this.camera.left = -frustumHeight * aspect / 2;
			_this.camera.right = frustumHeight * aspect / 2;
			_this.camera.top = frustumHeight / 2;
			_this.camera.bottom = -frustumHeight / 2;
			_this.camera.updateProjectionMatrix();
			...
			var cameraZ = _this.zoom;
			if (_this.zoom < 0) cameraZ = _this.calc_z_for_auto_zoom();
			_this.camera.position.set(_this.camera.position.x, _this.camera.position.y, cameraZ);
	...
	this.calc_z_for_auto_zoom = function (offset)
	{
		offset = offset || 1.1; // Small margin
		
		const boundingBox = new THREE.Box3(
			new THREE.Vector3(_this.minx, _this.miny, _this.minz), 
			new THREE.Vector3(_this.maxx, _this.maxy, _this.maxz)
		);
		
		var size = new THREE.Vector3();
		boundingBox.getSize(size);
		
		// For orthographic camera, we return the frustum height needed to contain the model
		var maxModelSize = Math.max(size.x, size.y, size.z);
		return maxModelSize * offset;
	}
	...
	this.set_camera_state = function (state)
{
    ...
    if (state.position !== undefined) {
        ...
        if (_this.camera instanceof THREE.OrthographicCamera) {
            // For orthographic, Z position doesn't affect view, but we store it for frustum
            _this.currentFrustumHeight = Math.max(state.position.z, 0.1);
            _this.do_resize();

            _this.camera.position.set(state.position.x, state.position.y, 10);
        } else {
            _this.camera.position.set(state.position.x, state.position.y, state.position.z);
        }
    }
    
    // ... rest of the existing set_camera_state code ...
}
	this.set_camera = function (x, y, z)
	{
		if (_this.camera instanceof THREE.OrthographicCamera) {
			// For orthographic, we can't set Z position the same way
			// Instead adjust the frustum size
			if (!_this.is_empty(z)) {
				_this.currentFrustumHeight = Math.max(z, 0.1);
				_this.do_resize(); // This will update the camera with new frustum
			}
			_this.camera.position.set(
				!_this.is_empty(x) ? x : _this.camera.position.x,
				!_this.is_empty(y) ? y : _this.camera.position.y,
				10 // Fixed Z position for orthographic
			);
		} else {
			// Original perspective camera code
			if (y) _this.zoom = y;
			_this.camera.position.set(
				!_this.is_empty(x) ? x : _this.camera.position.x,
				!_this.is_empty(y) ? y : _this.camera.position.y,
				!_this.is_empty(z) ? z : _this.camera.position.z
			);
		}
	}
	...
	this.do_resize = function()
	{
		...
		if (_this.camera instanceof THREE.OrthographicCamera) {
			// For orthographic camera, update the frustum based on current zoom
			var aspect = rsize_width / rsize_height;
			var frustumHeight = _this.currentFrustumHeight || 2; // Default initial value
			...
			// Keep original perspective camera code
			_this.camera.aspect = rsize_width / rsize_height;
			_this.camera.updateProjectionMatrix();
		}
	...
	this.init=function()
	{
		...
		if (!_this.init_done) //one time only
		{
			...
			// const aspect = _this.parent_element.clientWidth / _this.parent_element.clientHeight;
			// const frustumSize = 1000; // Adjust this value as needed
			...
			var width = _this.parent_element.clientWidth || 800;
			var height = _this.parent_element.clientHeight || 600;
			var aspect = width / height;
			
			// Create orthographic camera with initial values (will be updated in set_zoom)
			_this.camera = new THREE.OrthographicCamera(
				-1 * aspect,  // left
				1 * aspect,   // right
				1,            // top
	...
	_this.currentFrustumHeight = 2; // Default initial frustum height
